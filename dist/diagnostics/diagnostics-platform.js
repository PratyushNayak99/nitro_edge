...
function normaliseError(error) {
    ...
    if (value?.code === 'ETIMEDOUT' || value?.killed || value?.signal === 'SIGTERM')
...
const SCENARIOS = ['nominal', 'network-down', 'bluetooth-blocked', 'service-failed', 'minimal-host'];
export class DiagnosticsPlatform {
    runner;
    capabilities;
    simulation = process.env.EDGE_SENTINEL_MODE === 'simulation';
    scenario = SCENARIOS.includes(process.env.EDGE_SENTINEL_SCENARIO)
        ? process.env.EDGE_SENTINEL_SCENARIO
        : 'nominal';
    constructor(runner = new HostCommandRunner()) {
        this.runner = runner;
    }
    async onModuleInit() {
    ...
    result(started, strategy, fields) {
        return { ...fields, execution: { mode: this.simulation ? 'simulation' : 'host', strategy, elapsed_ms: Date.now() - started } };
    }
    async caps() { return this.refreshCapabilities(); }
    failure(started, strategy, capabilities, error, summary) {
    ...
    async systemHealth() {
        ...
        const capabilities = await this.caps();
        try {
            ...
            if (!uptime)
                return this.failure(started, 'none', capabilities, 'command_unavailable', 'No portable system telemetry source is available.');
            ...
            return this.failure(started, '/proc telemetry', capabilities, normaliseError(error), 'System telemetry could not be read.');
        }
    }
    async networkState() {
        const started = this.start();
        const capabilities = await this.caps();
        if (this.simulation && this.scenario === 'network-down')
            return this.result(started, 'ip -j link', { status: 'degraded', summary: 'No active network interface was detected.', capabilities, observations: [{ code: 'network_unavailable', severity: 'critical', message: 'NetworkManager is unavailable and no active link was found.' }], remediation: ['Check Ethernet/Wi-Fi link state and network configuration before retrying.'] });
        try {
            ...
            if (capabilities.ip) {
                ...
                const links = ip && !this.simulation ? JSON.parse((await this.runner.run(ip, ['-j', 'link'])).stdout || '[]') : [{ ifname: 'eth0', operstate: 'UP' }];
            ...
            return this.failure(started, 'network diagnostic', capabilities, normaliseError(error), 'Network state could not be inspected.');
        }
    }
    async bluetoothState() {
        ...
        if (absent || (!capabilities.rfkill && !capabilities.bluetooth_sysfs))
            return this.result(started, 'none', { status: 'unavailable', summary: 'Bluetooth hardware is not present.', capabilities, observations: [{ code: 'hardware_not_present', severity: 'info', message: 'No Bluetooth adapter capability was detected.' }], remediation: remediationFor.hardware_not_present, error: 'hardware_not_present' });
        if (this.simulation && this.scenario === 'bluetooth-blocked')
            return this.result(started, 'rfkill', { status: 'degraded', summary: 'Bluetooth adapter is soft-blocked.', capabilities, observations: [{ code: 'bluetooth_soft_blocked', severity: 'warning', message: 'An adapter is present but software radio blocking is active.' }], remediation: ['Clear the Bluetooth soft block only if policy allows it, then rerun this read-only check.'] });
        try {
            if (capabilities.rfkill) {
                ...
                if (!output)
                    return this.result(started, 'rfkill', { status: 'unavailable', summary: 'Bluetooth hardware is not present.', capabilities, observations: [{ code: 'hardware_not_present', severity: 'info', message: 'rfkill found no Bluetooth adapter.' }], remediation: remediationFor.hardware_not_present, error: 'hardware_not_present' });
            ...
            return this.result(started, 'Bluetooth sysfs', { status: adapters.length ? 'healthy' : 'unavailable', summary: adapters.length ? 'Bluetooth adapter is present.' : 'Bluetooth hardware is not present.', capabilities, observations: [{ code: adapters.length ? 'bluetooth_available' : 'hardware_not_present', severity: 'info', message: adapters.length ? 'Used Bluetooth sysfs fallback.' : 'No Bluetooth adapter was found.' }], remediation: adapters.length ? [] : remediationFor.hardware_not_present, ...(adapters.length ? {} : { error: 'hardware_not_present' }) });
        }
        catch (error) {
            return this.failure(started, 'bluetooth diagnostic', capabilities, normaliseError(error), 'Bluetooth state could not be inspected.');
        }
    }
    async serviceState(serviceName, action) {
        const started = this.start();
        const capabilities = await this.caps();
        if (!capabilities.systemctl && !capabilities.service)
            return this.result(started, 'none', { status: 'unavailable', summary: 'No supported service manager is available.', capabilities, observations: [{ code: 'service_manager_unavailable', severity: 'warning', message: 'Neither systemctl nor service is available.' }], remediation: remediationFor.service_manager_unavailable, error: 'service_manager_unavailable' });
        ...
        try {
            ...
            if (!executable)
                return this.failure(started, strategy, capabilities, 'command_unavailable', 'The selected service manager is unavailable.');
            ...
            if (action === 'restart')
                await this.runner.run(executable, strategy === 'systemctl' ? ['restart', serviceName, '--no-pager'] : [serviceName, 'restart']);
    ...
    async kernelLogs() {
        const started = this.start();
        const capabilities = await this.caps();
        if (!capabilities.kernel_logs)
            return { content: 'Kernel logs are unavailable on this host.', result: this.failure(started, 'none', capabilities, 'command_unavailable', 'Kernel log access is unavailable.') };
        if (this.simulation)
            return { content: 'simulation: kernel log access available', result: this.result(started, 'dmesg --lines 50', { status: 'healthy', summary: 'Kernel log access is available.', capabilities, observations: [{ code: 'kernel_logs_available', severity: 'info', message: 'Simulation kernel log fixture returned.' }], remediation: [] }) };
        try {
            ...
            const code = normaliseError(error);
            return { content: 'Kernel logs are unavailable on this host.', result: this.failure(started, 'dmesg --lines 50', capabilities, code, 'Kernel log access could not be read.') };
        }
    }
}
//# sourceMappingURL=diagnostics-platform.js.map